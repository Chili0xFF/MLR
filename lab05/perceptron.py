...
class simple_perceptron:
    ...
    def train(self, Xtrain, Ytrain, Xvalid, Yvalid):
        ...
        for epoch in range(self.epochs):
            ...
            for i in range(len(Xtrain)):
                ...
                for j in range(self.Nin):
                    # yi = 
                    change = -1 * self.learning_rate*self.fp(sumWeighted)*(Yout-Ytrain[i])*Xtrain[i][j]
                    self.weights[j] += change # ZADANIE - uzupełnić prawą stronę wzoru na zmianę wagi (2p) 
    ...
    def save_model(self, filename):
        ...
        Returns:
            None
        self.nin, min_val, max_val, activation
        optional: epoch, learning_rate
        '''
        data = dict(
            nin = self.Nin,
        ...
        # ZADANIE - opracować i zaimplementować dowolny sposób na zapis parametrów modelu do pliku (1p)
    ...
    def load_model(self, filename):
        ...
        # ZADANIE - zaimplementować odczyt parametrów modelu z pliku (1p)
