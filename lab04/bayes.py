...
for ih, h in enumerate(data["Hypotheses"]):
    ...
    iloczyn = 1
    for f in data["Facts"]:
        probabilityOfAFact = f["prob"][ih]
        iloczyn = iloczyn * probabilityOfAFact
    numerator = numerator * iloczyn
    
    # Calculate the denominator of the formula
    denumerator = 0 # Should be replaced by the actual calculations
    for ih2, h2 in enumerate(data["Hypotheses"]):
        prh = h2["prob"]
        iloczyn = 1
        for f in data["Facts"]:
            iloczyn = iloczyn * f["prob"][ih2]
        prh *= iloczyn
        denumerator += prh 
